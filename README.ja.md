...
`examples/` は**イニシャルデモ**です。すでに end-to-end で動く 2 つのノートブックを、
この表現で何ができるかを示すショーケースとして置いています。
**今後の開発は次の 4 ディレクトリで進めます**（各ディレクトリに README があります）。
...
`tool/` は工程横断のディレクトリです。生成時・アダプタ実装時・実験時のいずれであっても、
...
コンセプト・アーキテクチャ・ロードマップ・制限事項は [`docs/`](docs/) にあります。
